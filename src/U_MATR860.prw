...
/* 
// Comente ou apague este bloco para o aviso sumir:
...
SELECT SD3.*,
       ...
       CASE WHEN SB1.B1_COD LIKE 'MOD%' OR SB1.B1_CCCUSTO <> ' ' THEN 'S' ELSE 'N' END ISMOD
...
//³Totais Gerais                            ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0028,/*Picture*/,{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQUISICOES ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0029,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQUISICOES ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0030,/*Picture*/,{|| If(SubStr((cQryOP)->D3_CF,1,2)=="PR",oSection1:Cell('D3_QUANT'):GetValue(),0) },.F.,.T.) //"QTDE. TOTAL PRODUCAO ---------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0031,PesqPict("SD3","D3_CUSTO1",17),{|| If(SubStr((cQryOP)->D3_CF,1,2)=="PR",oSection1:Cell('Custo'   ):GetValue(),0) },.F.,.T.) //"CUSTO TOTAL PRODUCAO ---------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0032,/*Picture*/,{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEVOLUCOES  ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0033,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEVOLUCOES  ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0034,/*Picture*/,{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1)=="9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQ PODER 3 ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0035,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1)=="9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQ PODER 3 ------->"
                                                                        
oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0036,/*Picture*/,{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1)=="9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEV PODER 3 ------->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0037,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'N' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1)=="9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEV PODER 3 ------->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0038,/*Picture*/,{|| If((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", oSection1:Cell('D3_QUANT'):GetValue(), 0) },.F.,.T.) //"QTDE. TOTAL REQ. MAO DE OBRA -->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0039,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="RE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", oSection1:Cell('Custo'   ):GetValue(), 0) },.F.,.T.) //"CUSTO TOTAL REQ. MAO DE OBRA -->"

oFunction := TRFunction():New(oSection1:Cell('D3_QUANT'),NIL,"SUM",NIL,STR0040,/*Picture*/,{|| If((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", (- oSection1:Cell('D3_QUANT'):GetValue()), 0) },.F.,.T.) //"QTDE. TOTAL DEV. MAO DE OBRA -->"
oFunction := TRFunction():New(oSection1:Cell('Custo'   ),NIL,"SUM",NIL,STR0041,PesqPict("SD3","D3_CUSTO1",17),{|| If((cQryOP)->ISMOD == 'S' .And. SubStr((cQryOP)->D3_CF,1,2)=="DE" .And. SubStr((cQryOP)->D3_CF,3,1) # "9", (- oSection1:Cell('Custo'   ):GetValue()), 0) },.F.,.T.) //"CUSTO TOTAL DEV. MAO DE OBRA -->"
...
While !oReport:Cancel() .And. !(cQryOP)->(Eof())
	...
	While (cQryOP)->D3_FILIAL+(cQryOP)->D3_OP == xFilial("SD3")+cOpAnt
		...
		nValUnit := 0
		If (cQryOP)->D3_QUANT <> 0
			nValUnit := &(cCampoCus) / (cQryOP)->D3_QUANT
		EndIf
		oReport:Section(1):Cell('Unitario'):SetValue(nValUnit)
		// MATEUS HENGLE - 31/05/23
...
While lContinua .And. !Eof()
	...
	While !Eof() .And. D3_FILIAL+D3_OP = xFilial()+cOpAnt
		...
		If SubStr(D3_CF,1,2) == "DE"
			...
			@ Li,aColPos[08] PSay IIf(D3_QUANT == 0, 0, nCusto/D3_QUANT)	Picture PesqPict("SD3","D3_CUSTO1",17)
